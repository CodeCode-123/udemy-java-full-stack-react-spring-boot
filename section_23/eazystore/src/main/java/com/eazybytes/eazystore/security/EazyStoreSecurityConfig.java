...
public class EazyStoreSecurityConfig {
    ...
    SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        //return http.csrf(csrfConfig -> csrfConfig.disable()) // disable csrf protect for all urls is dangerous
        return http.csrf(csrfConfig ->
                // CsrfTokenRepository that persists the CSRF token in a cookie named "XSRF-TOKEN" and reads
                // from the header "X-XSRF-TOKEN" following the convention of AngularJS. When using with
                // AngularJS be sure to use withHttpOnlyFalse()
                        csrfConfig.csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse())
                                // csrfTokenRequestHandler makes the CsrfToken available to the application as a request
                                // attribute and resolving the token value from incoming HTTP requests
